  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,_rgba(37,99,235,0.15),transparent_50%)]"></div>
        <div className="max-w-6xl mx-auto px-6 py-20 relative z-10">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            {/* Izquierda - Copy */}
            <div>
              <div className="inline-flex items-center gap-2 bg-primary/10 border border-primary/20 text-primary text-sm font-bold px-4 py-2 rounded-full mb-6">
                <Zap size={16} /> COTIZADOR LÁSER SAAS
              </div>
              <h1 className="text-4xl md:text-5xl font-black leading-tight mb-6">
                Cotizaciones automáticas para tu taller de <span className="text-primary">corte láser</span>
              </h1>
            ...
            {/* Derecha - Auth Form */}
            <div className="bg-dark border border-slate-700 p-8 rounded-2xl shadow-2xl">
              ...
              <form onSubmit={handleAuth} className="space-y-4">
                <div>
                  ...
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white focus:border-primary outline-none" required />
                </div>
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase">Contraseña</label>
                  <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white focus:border-primary outline-none" required />
                </div>
                <button disabled={loading} className="w-full bg-primary hover:bg-primary/80 disabled:bg-slate-700 text-white font-bold py-4 rounded-xl transition-all flex items-center justify-center gap-2">
...
function OnboardingPage({ setEmpresa }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
      <div className="bg-dark border border-slate-800 p-8 rounded-2xl max-w-lg w-full">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-gradient-to-br from-primary to-primary/80 rounded-2xl mx-auto flex items-center justify-center mb-4">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input value={form.nombre} onChange={e => setForm({ ...form, nombre: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white focus:border-primary outline-none" required />
          </div>
          <div>
            <label className="text-xs font-bold text-slate-500 uppercase">Slogan</label>
            <input value={form.slogan} onChange={e => setForm({ ...form, slogan: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white focus:border-primary outline-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase">Teléfono</label>
              <input value={form.telefono} onChange={e => setForm({ ...form, telefono: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white focus:border-primary outline-none" />
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 uppercase">Email</label>
              <input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white focus:border-primary outline-none" />
            </div>
          </div>
          <div>
            <label className="text-xs font-bold text-slate-500 uppercase">Dirección</label>
            <input value={form.direccion} onChange={e => setForm({ ...form, direccion: e.target.value })} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-white focus:border-primary outline-none" />
          </div>
          <button disabled={saving} className="w-full bg-primary hover:bg-primary/80 disabled:bg-slate-700 text-white font-bold py-4 rounded-xl transition-all flex items-center justify-center gap-2">
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <div className="bg-primary/10 border-b border-primary/20 px-6 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2 text-sm">
            ...
            <code className="bg-dark px-3 py-1 rounded text-primary font-mono">{publicUrl}</code>
          </div>
          <button onClick={copyUrl} className="flex items-center gap-2 text-primary hover:text-primary/80 text-sm font-bold">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        <div className="flex gap-4 mb-8 border-b border-slate-800 pb-1">
          <button onClick={() => setTab('pedidos')} className={`px-4 py-2 font-bold text-sm transition-all border-b-2 ${tab === 'pedidos' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-white'}`}>
            Pedidos Recientes
          </button>
          <button onClick={() => setTab('materiales')} className={`px-4 py-2 font-bold text-sm transition-all border-b-2 ${tab === 'materiales' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-white'}`}>
            Materiales
          </button>
          <button onClick={() => setTab('empresa')} className={`px-4 py-2 font-bold text-sm transition-all border-b-2 ${tab === 'empresa' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-white'}`}>
            Configuración
          </button>
          <button onClick={() => setTab('seguridad')} className={`px-4 py-2 font-bold text-sm transition-all border-b-2 ${tab === 'seguridad' ? 'border-primary text-primary' : 'border-transparent text-slate-400 hover:text-white'}`}>
            Seguridad
          </button>
        </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={cargarPedidos} className="text-slate-400 hover:text-primary text-sm flex items-center gap-1">
              ...
              {pedidos.map((p) => (
                <tr key={p.id} className="hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="p-4 text-right flex items-center justify-end gap-2">
                    {p.archivo_url && (
                      <a href={p.archivo_url} target="_blank" rel="noreferrer" className="bg-slate-700 hover:bg-primary hover:text-white text-slate-200 p-2 rounded-lg">
        ...
        <form onSubmit={handleSave}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-primary/10 p-4 rounded-lg border border-primary/30 space-y-3">
              <h4 className="text-sm font-bold text-primary">Suministro de Material (Opcional)</h4>
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="text-xs font-bold text-primary uppercase block mb-1">Precio Venta</label>
                ...
                <div className="w-1/3">
                  <label className="text-xs font-bold text-primary uppercase block mb-1">Unidad</label>
          ...
          <div className="flex justify-end mt-6">
            <button type="button" onClick={handleSave} disabled={saving} className="bg-primary hover:bg-primary/80 text-white font-bold py-3 px-8 rounded-lg flex items-center justify-center gap-2">
...
function AdminEmpresa({ empresa, setEmpresa }) {
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 max-w-2xl">
      <h3 className="font-bold mb-6 flex items-center gap-2"><Building2 size={20} className="text-primary" /> Datos de la Empresa</h3>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div>
          ...
          <div className="bg-slate-900 border border-slate-600 rounded-lg p-4 text-center">
            ...
            <label className="cursor-pointer bg-primary hover:bg-primary/80 text-white text-xs font-bold px-4 py-2 rounded-lg inline-flex items-center gap-2">
      ...
      <button onClick={handleSave} disabled={saving || uploading} className="mt-6 bg-primary hover:bg-primary/80 disabled:bg-slate-700 text-white font-bold px-6 py-3 rounded-xl flex items-center gap-2">
...
function AdminSeguridad() {
  ...
  return (
    <div className="bg-slate-800 p-6 rounded-xl border border-slate-700 max-w-md">
      <h3 className="font-bold mb-6 flex items-center gap-2"><Lock size={20} className="text-primary" /> Cambiar Contraseña</h3>
      ...
      <div className="space-y-4">
        ...
        <button onClick={handleChangePassword} disabled={loading} className="w-full bg-primary hover:bg-primary/80 disabled:bg-slate-700 text-white font-bold px-6 py-3 rounded-xl flex items-center justify-center gap-2">
      ...
      <div className="w-full md:w-[420px] bg-slate-800 flex flex-col border-r border-slate-700">
        <div className="p-6 border-b border-slate-700 bg-slate-900">
          <div className="flex items-center gap-4">
            {(empresa.faviconUrl || empresa.favicon_url) ? (
              ...
              <div className="w-12 h-12 bg-secondary rounded-lg flex items-center justify-center font-black text-slate-900">
            ...
            <div className="flex-1">
              ...
              <span className="text-secondary text-xs font-bold uppercase">{empresa.slogan}</span>
            </div>
          </div>
        ...
        <div className="p-6 flex-1 flex flex-col">
          ...
          <div className="space-y-2 mb-4">
            <div className="bg-secondary text-slate-900 p-3 px-4 flex justify-between items-center rounded font-black text-sm">
            ...
            <div className="mb-4 bg-primary/10 border border-primary/30 rounded-xl p-4">
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-5 h-5 bg-slate-900 border-2 border-primary rounded checked:bg-primary cursor-pointer"
                />
                <div className="flex-1">
          ...
          <label className="group relative border-2 border-dashed border-primary/50 rounded-2xl flex-1 min-h-[180px] flex flex-col items-center justify-center cursor-pointer hover:border-primary hover:bg-slate-700/30 transition-all">
            <input
              type="file"
              className="hidden"
              accept=".dxf,.svg"
            ...
            {procesando ? (
              <div className="flex flex-col items-center">
                ...
                <span className="text-primary font-bold text-sm">PROCESANDO...</span>
              </div>
            ) : (
              <>
                <Upload className="text-primary mb-3" size={36} />
        ...
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl max-w-lg w-full shadow-2xl">
          ...
          <div className="space-y-4 mb-6">
            <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800 flex justify-between">
              <span className="text-slate-400 text-xs font-bold uppercase flex items-center gap-2">
                <FileText size={14} className="text-primary" /> Archivo
              </span>
            ...
            <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800">
              ...
              <div className="flex items-center justify-between bg-slate-900 rounded-lg p-1 border border-slate-800">
                ...
                <button
                  ...
                  className="w-10 h-10 bg-primary text-white rounded-lg flex items-center justify-center hover:bg-primary/80"
                >
                  <Plus size={16} />
                </button>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800">
                ...
                <div className="text-secondary font-mono text-lg font-bold">
            ...
            {incluyeMaterial && areaCm2 > 0 && (
              <div className="bg-primary/10 border border-primary/30 p-4 rounded-xl">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-primary text-xs font-bold uppercase flex items-center gap-2">
          ...
          <div className="flex gap-4">
            <button
              ...
              className="w-full bg-secondary hover:bg-secondary/80 disabled:opacity-50 text-slate-900 py-4 rounded-xl font-black uppercase shadow-lg shadow-secondary/20 transform hover:scale-[1.02] transition-all"
            >
              SOLICITAR CORTE
            </button>
          </div>
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-800 w-full max-w-2xl overflow-hidden shadow-2xl">
            <div className="flex justify-between items-center p-6 border-b border-slate-800 bg-slate-900">
              <h3 className="text-xl font-bold flex items-center gap-2 text-white">
                <Zap className="text-secondary" /> Confirmar Orden de Corte
              </h3>
              <button
              ...
              <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 mb-6 space-y-3">
                ...
                {incluyeMaterial && (
                  <div className="flex justify-between items-center pb-3 border-b border-slate-700">
                    ...
                    <span className="text-primary font-bold">{formatoPesos(costoMaterialUnitario * cantidad)}</span>
                  </div>
                )}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-700 rounded-lg p-3 text-white focus:border-primary outline-none"
            ...
            <div className="p-6 border-t border-slate-800 flex justify-end gap-3 bg-slate-900">
              ...
              <button
                ...
                className="bg-secondary hover:bg-secondary/80 text-slate-900 font-black px-8 py-3 rounded-xl flex items-center gap-2"
              >
