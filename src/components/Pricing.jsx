...
import { useNavigate, useLocation } from 'react-router-dom'; // <--- IMPORTANTE: useLocation
...
export default function Pricing({ isLocked = false }) {
    ...
    // LÓGICA INTELIGENTE:
    // ¿Estamos bloqueados? O ¿La URL dice '?mode=renew'?
    const queryParams = new URLSearchParams(location.search);
    const isRenewing = queryParams.get('mode') === 'renew';

    // Si está bloqueado O está renovando, mostramos MODO PAGO (Verde)
    const showPaymentMode = isLocked || isRenewing;
    ...
    const calcularPrecio = (base) => {
        let monto = base;
        if (anual) monto = (monto * 12) * 0.8;
        const final = monto * TASAS[moneda].factor;
        return new Intl.NumberFormat('es-CO', { style: 'currency', currency: moneda === 'COP' ? 'COP' : 'USD', maximumFractionDigits: 0 }).format(final).replace('COP', '').trim();
    };

    const handleAction = (planNombre, precioBase, mpLink) => {
        if (showPaymentMode) { // <--- Usamos la nueva variable
            setSelectedPlan({ name: planNombre, price: calcularPrecio(precioBase), mpLink: mpLink });
    ...
    const copiarPortapapeles = (texto) => { navigator.clipboard.writeText(texto); alert('Copiado: ' + texto); };
        ...
        <section className="py-12 md:py-24 bg-zinc-900 border-t border-zinc-800 relative overflow-hidden min-h-screen">
            ...
            <div className="max-w-6xl mx-auto px-6 relative z-10">
                ...
                <div className="pt-4 mb-12">
                    <button onClick={() => navigate('/')} className="group flex items-center gap-3 px-5 py-2.5 bg-zinc-950/50 backdrop-blur-md border border-zinc-800 rounded-full hover:border-amber-500/50 hover:bg-zinc-900 transition-all duration-300 w-fit">
                        ...
                        <span className="text-zinc-400 text-xs font-black uppercase tracking-widest group-hover:text-white transition-colors">Volver al Inicio</span>
                    </button>
                </div>

                <div className="text-center max-w-3xl mx-auto mb-16">
                    <h2 className="text-3xl md:text-5xl font-black text-white mb-6 leading-tight">
                        {showPaymentMode ? <span className="text-green-500">Renueva tu Servicio</span> : <>Potencia tu taller <br /><span className="text-amber-500">sin romper la alcancía</span></>}
                    </h2>
                    <p className="text-zinc-400 text-lg mb-8">
                        {showPaymentMode ? "Elige tu plan para continuar disfrutando de todos los beneficios." : "Empieza hoy con nuestra prueba de 21 días con TODO incluido."}
                    </p>

                    {/* ... (Aquí van los botones de Mensual/Anual y Moneda, igual que antes) ... */}
                ...
                <div className="grid md:grid-cols-3 gap-8 items-start">
                    ...
                    <div className="bg-zinc-950 border border-zinc-800 rounded-2xl p-8 hover:border-zinc-600 transition-all duration-300">
                        ...
                        <button
                            ...
                            className={`w-full py-3 font-bold rounded-lg transition-colors mb-8 border ${showPaymentMode ? 'bg-green-600 hover:bg-green-500 text-white border-green-500' : 'bg-zinc-800 hover:bg-zinc-700 text-white border-zinc-700'}`}
                        >
                            {showPaymentMode ? 'PAGAR AHORA' : 'PRUEBA 21 DÍAS GRATIS'}
                        </button>
                    ...
                    <div className="bg-zinc-900 border-2 border-amber-500 rounded-2xl p-8 relative shadow-2xl shadow-amber-500/10 transform md:-translate-y-4">
                        ...
                        <button
                            ...
                            className={`w-full py-4 font-black rounded-lg transition-colors mb-8 shadow-lg ${showPaymentMode ? 'bg-green-500 hover:bg-green-400 text-black' : 'bg-amber-500 hover:bg-amber-400 text-black'}`}
                        >
                            {showPaymentMode ? 'PAGAR AHORA' : 'PRUEBA 21 DÍAS GRATIS'}
                        </button>
            ...
            {/* --- MODAL DE PAGOS (El mismo de antes) --- */}
                ...
                <div className="fixed inset-0 z-[60] bg-black/90 backdrop-blur-sm flex items-center justify-center p-4">
                    <div className="bg-zinc-900 w-full max-w-lg rounded-xl border border-zinc-800 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="p-6 border-b border-zinc-800 flex justify-between items-center bg-zinc-950">
                            <div><h3 className="text-xl font-black text-white uppercase tracking-wide">Realizar Pago</h3><p className="text-zinc-400 text-sm mt-1">{selectedPlan.name} • <span className="text-amber-500 font-bold">{selectedPlan.price}</span></p></div>
                        ...
                        <div className="p-6 space-y-6">
                            <div className="bg-zinc-950/50 border border-blue-500/30 rounded-lg p-5 hover:border-blue-500 transition-colors group">
                                <div className="flex items-center gap-4 mb-4"><div className="bg-blue-500/10 p-3 rounded-lg"><CreditCard className="text-blue-500" size={24} /></div><div><h4 className="font-bold text-white">Mercado Pago / Tarjeta / PSE</h4><p className="text-zinc-500 text-xs">Activación automática (Próximamente)</p></div></div>
                                <a href={selectedPlan.mpLink} target="_blank" rel="noreferrer" className="block w-full text-center bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-lg transition-colors">PAGAR CON MERCADO PAGO</a>
                            </div>
                            <div className="bg-zinc-950/50 border border-purple-500/30 rounded-lg p-5 hover:border-purple-500 transition-colors">
                                <div className="flex items-center gap-4 mb-4"><div className="bg-purple-500/10 p-3 rounded-lg"><Smartphone className="text-purple-500" size={24} /></div><div><h4 className="font-bold text-white">Nequi o Daviplata</h4><p className="text-zinc-500 text-xs">Transferencia manual</p></div></div>
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between bg-zinc-900 p-3 rounded border border-zinc-800"><span className="text-zinc-400 text-xs font-bold uppercase">Nequi</span><div className="flex items-center gap-2"><span className="text-white font-mono font-bold">{DATOS_NEQUI}</span><button onClick={() => copiarPortapapeles(DATOS_NEQUI)} className="text-zinc-500 hover:text-white"><Copy size={14} /></button></div></div>
                                    <div className="flex items-center justify-between bg-zinc-900 p-3 rounded border border-zinc-800"><span className="text-zinc-400 text-xs font-bold uppercase">Daviplata</span><div className="flex items-center gap-2"><span className="text-white font-mono font-bold">{DATOS_DAVIPLATA}</span><button onClick={() => copiarPortapapeles(DATOS_DAVIPLATA)} className="text-zinc-500 hover:text-white"><Copy size={14} /></button></div></div>
                                </div>
                            </div>
                            <div className="text-center pt-2"><p className="text-xs text-zinc-500">Una vez realizado el pago, envía el comprobante a soporte para activar tu cuenta inmediatamente.</p></div>
                        </div>
                    </div>
                </div>
            )}
